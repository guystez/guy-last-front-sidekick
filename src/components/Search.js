...
const username = localStorage.getItem('username');
  ...
  return (
    ...
    <div class="top-buttons-wrapper">
      <button class="openers_button active" onClick={() => handleOptionSelect('search')}>צ'אט</button>
      <button class="openers_button" onClick={() => handleOptionSelect('openers')}>משפטים</button>
    </div>
  ...
  <div key={index}>
        ...
        <div className='entrance'>
          ...
          <button onClick={() => sendToCart(item.response, localStorage.getItem('username'))}>Like</button>
            ...
            <form onSubmit={phrase_extraction} className='search-input'>
              <input type="text" value={searchText} onChange={handleSearchChange} placeholder="שלח הודעה..." />
              <button className="submit-search" type='submit' ref={pageRef}>שלח</button>
              <button className="community-button" type="button" onClick={submitNewQuestion}>שאל משתמשים</button>
            </form>
            <div className="chatborderButtonDiv">
          ...
          <button onClick={() => sendToCart(item.users_answer, localStorage.getItem('username'))}>Like</button>
            ...
            <div className="search-container">
              <div className="cart-container">
                <h2 className="favorite-chat-and-openers">שיחות שאהבת</h2>
                  ...
                  {item.chat.chat}{item.chat.id} */}
                    {cart.map(item => (
                      <tr key={item.id} className="cart-table-row">
                        <td className="cart-table-cell">{item.chat.chat} <button onClick={() => handleDelete(item.chat.id)}>מחק</button></td>
            ...
            <div className="search-container">
              ...
              <div>
              <button className="new-chat-button" onClick={handleClick}>צ'אט חדש</button>
    ...
    <tr key={index} className="cart-table-row2">
        ...
        <td className="cart-table-cell2">
            ...
            <button onClick={() => deleteChatRoom(roomId)}>מחק</button>
        </td>
    </tr>
))}
